...
for idx, row in data.iterrows():
    ...
    matches_info = data.iloc[top_indices][['Name', 'Gender', 'Branch', 'Field of Study', 'Hobbies', 'Preferred Language', 'Predicted MBTI']].to_dict(orient='records')
    top_matches[row['Name']] = matches_info

# Add top matches to the DataFrame
data["Top Matches"] = data["Name"].map(top_matches)

# Save the results to a new CSV file
data.to_csv('Responses_Top_Matches.csv', index=False)

print("Top matches have been generated and saved to 'Responses_giTop_Matches.csv'.")
